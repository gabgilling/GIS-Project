...
cces$GEOID <- paste(as.character(cces$inputstate), as.character(cces$cdid116), sep = "")
...
cces2$GEOID <- paste(as.character(cces2$inputstate), as.character(cces2$cdid116), sep = "")
...
t2 <- cces2 %>% group_by(GEOID) %>% summarise(pref_index2 = mean(q5+q6+q7+q8+q9, na.rm = T))

t1 <- cces %>% group_by(GEOID) %>% summarise(pref_index1 = mean(q1+q2+q3+q4, na.rm = T))

# recode 4 (neither) as 0
cces$pid3 <- ifelse(cces$pid3 %in% c(4,3), 0, cces$pid3)
cces$pid3 <- ifelse(cces$pid3 == 5, NA, cces$pid3)

cces2$pid3 <- ifelse(cces2$pid3 %in% c(4,3), 0, cces2$pid3)
cces2$pid3 <- ifelse(cces2$pid3 == 5, NA, cces2$pid3)

pid <- cces2 %>% group_by(GEOID) %>% summarise(pid = mean(pid3, na.rm = T))

f <- merge(t1, t2, by = "GEOID", all = T)
...
f <- merge(f %>% select(GEOID, pref_index), pid, by = "GEOID", all = T)

# prevent excel from thinking it's a number
f$GEOID <- paste("'", f$GEOID, sep ="")

write.xlsx(f, "/Users/gabgilling/Documents/GitHub/GIS-Project/f.xls")

cd_shp <- read.dbf("/Users/gabgilling/Downloads/tl_2018_us_cd116 (1)/tl_2018_us_cd116.dbf")
# 
# cd_shp$pref_index <- NULL
# cd_shp$pid <- NULL
